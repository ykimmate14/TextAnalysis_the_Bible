...
OTfreqPlot <- function(txt){
    ...
    str_grep <- grepl(text, verselist[old.t.index], ignore.case = TRUE)
...
NTfreqPlot <- function(txt){
    ...
    str_grep <- grepl(text, verselist[new.t.index], ignore.case = TRUE)
...
freqTable <- function(txt){
    ...
    for(i in 1:nrow(dt)){
        ...
        countlist <- c(countlist, sum(grepl(text, verselist[index], ignore.case = TRUE)))
    }
